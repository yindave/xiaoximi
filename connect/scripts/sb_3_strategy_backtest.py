...
# TODO: need to re-write later with more powerful PC to parallel run all the variations (not the shift perhaps?)
def run(shift_no):
    
    print ('working on shift_no %s' % (shift_no))
    ...
    dump_name='shift_%s' % (shift_no)
    
    dump_path=sc.path+'trades_and_models\\dump_for_bt\\'
...
if __name__ == "__main__":
    ...
    p_collector=[]
    for shift_no in shift_nos:
        p=Process(target=run,args=(shift_no,))
