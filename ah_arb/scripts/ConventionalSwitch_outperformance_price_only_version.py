...
parser.add_argument('--idx_ticker',dest='idx_ticker',type=str,default='HSCEI')
parser.add_argument('--level',dest='level',type=float,default=1)
...
level=round(float(args.level),1)


idx_ticker=idx_ticker_short+' Index'

dump_name='ConventionalSwitch_%s_%s' % (idx_ticker_short,level)
...
### get nice trades record ready
    ratios=ah.db['ratio'].resample('BM').last().stack().rename('ratio').reset_index()
    ratios['ticker_h']=ratios['ticker']
    ratios['ticker_a']=ratios['ticker'].map(lambda x: ha_map[x])
    ratios['ticker']=ratios.apply(lambda x: x['ticker_h'] if x['ratio']>level else x['ticker_a'],axis=1) # this is the ticker to swap out
    ratios['ticker_pair']=ratios.apply(lambda x: x['ticker_h'] if x['ticker']==x['ticker_a'] else x['ticker_a'],axis=1)
    ratios['trade_date']=ratios['date']
    ratios['exit_date']=ratios['trade_date']+pd.tseries.offsets.BMonthEnd(n=1)
    ratios['direction']=ratios['ticker'].map(lambda x: 'buy' if '-CN' in x else 'sell')
    trades_to_switch=ratios.set_index(['date','ticker'])[['ticker_pair','direction','trade_date','exit_date']]
### load index compo
