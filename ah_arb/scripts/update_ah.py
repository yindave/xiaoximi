...
'''
We no longer calculate the distance metrics (useless and time consuming)
We combine with update_ah_quick where we update the backtest
'''
...
success_1=um.retry_func(ah.update_ah_map,retries=5,
               error_msg_print='Unknown error try again.',
               error_msg_email='AH update error: AH map error try again',)

success_2=um.retry_func(ah.refresh_mkt,retries=5,
               error_msg_print='Unknown error try again.',
               error_msg_email='AH update error: refresh mkt error try again',)

success_3=um.retry_func(ah.refresh_db,retries=5,
               error_msg_print='Unknown error try again.',
               error_msg_email='AH update error: refresh db error try again',)

error_free=success_1 and success_2 and success_3
if not error_free:
    um.quick_auto_notice('AH full update stopped due to error')
    import sys
    sys.exit()
...
ah.load_db()
#ah.dump_adf()
#ah.dump_pair_distance(include_dtw=False) # takes very long time to run dtw
...
'''
Create the df that shows non-trading days for buy & sell
- any leg of the turnover is 0
- for entry
    - buy HA: cannot trade when A is limit down
    - sell HA: cannot trade when A is limit up
- for exit
    - buy HA: cannot trade when A is limit up
    - sell HA: cannot trade when A is limit down
Also note that we can't accuratly model the circuit break because we don't know
when it's hit during intraday. We just assume

Also we need to factor in the new circuit breaker 20% for STAR and ChiNext (after 24 Aug 2020)
'''
...
cb_levels=pd.concat([
                cb_levels.loc[:datetime(2020,8,23)].fillna(cb_levels_old),
                cb_levels.loc[datetime(2020,8,24):].fillna(cb_levels_new),
            ],axis=0).stack()
...
#---- update the underlying data for dynamic index, dynamic index needs launcher to run
#from ah_arb.scripts import get_price_for_all_compo
#from ah_arb.scripts import DynamicIndex_outperformance_price_only_version
#from ah_arb.scripts import ConventionalSwitch_outperformance_price_only_version
...
for direction in directions:
    ...
    for ticker in all_tickers:
        ...
        last_exit_date=datetime(1999,1,1)
...
trades['feature_level']=trades['ha_level'].copy()
...
um.quick_auto_notice('AH update finished')
